...
def _generate_grant_ai_answer(
    *,
    ...
    "2. НЕ используй фразы: 'зависит', 'возможно', 'уточните' ЕСЛИ информация есть в контексте.\n"
    "3. Если в контексте указано, что поступление по ЕНТ — прямо так и скажи.\n"
    "4. НЕ придумывай вступительные экзамены, если их нет.\n"
    "5. Если информации в контексте недостаточно:\n"
    "   — НЕ говори, что информации нет\n"
    "   — кратко скажи, что обычно поступление проходит по ЕНТ\n"
    "   — направь пользователя в приёмную комиссию\n"
    "   — предложи связаться через контакты (телефон, сайт, email)\n\n"

    "ПОВЕДЕНИЕ:\n"
    "— Если вопрос про гранты: объясни варианты (гос. грант, внутренние скидки)\n"
    "— Если вопрос про поступление: сначала укажи ЕНТ, затем детали\n"
    "— Если программа уже указана в истории — НЕ переспрашивай\n"
    "— Если не хватает данных: вежливо направь в приёмную комиссию\n\n"
    "СТРОГО ЗАПРЕЩЕНО:\n"
    "— говорить 'нет информации', 'не найдено', 'неизвестно'\n"
